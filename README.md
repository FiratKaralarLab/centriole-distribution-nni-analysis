...
Detects the POSITION_X and POSITION_Y columns automatically.

Calculates the nearest-neighbor distance for each detected centriole.

Calculates the mean nearest-neighbor distance.

Estimates the expected nearest-neighbor distance under complete spatial randomness (CSR).

Calculates the Nearest Neighbor Index (NNI).

Classifies the spatial pattern as clustered, random, or dispersed.

Saves a scatter plot of centriole positions.

Saves a histogram of nearest-neighbor distances.

Generates a summary CSV file for all analyzed files.
...
Clustered, random, or dispersed classification
